---
title: "Mapping protein features to splice events"
shorttitle: "Mapping protein features to splice events"
author: "Diogo Veiga"
package: maser
date: "`r Sys.Date()`"
output:
  BiocStyle::html_document:
    toc_float: true  
vignette: >
  %\VignetteIndexEntry{Mapping protein features to splice events}
...
Integration of protein features to splicing events may reveal the impact of alternative splicing on protein function. We developed `r Rpackage("maser")` to enable systematic mapping of protein annotation from [UniprotKB](http://www.uniprot.org/) to splice events.  
...
Protein feature annotation of splice events is performed in two steps.
...
`mapTranscriptsToEvents()` requires an Ensembl or Gencode GTF using the hg38 build of the human genome. Ensembl GTFs can be retrieved using `r Biocpkg("AnnotationHub")` as below. Several releases are available, and `r Rpackage("maser")` is compatible with any version using the hg38 build.
...
In the second step, `mapProteinFeaturesToEvents()` retrieves data from UniprotKB and overlaps splice events to genomic coordinates of protein features. 


## SRSF6 example

The splicing factor SRSF6 undergoes splicing during hypoxia by expressing an alternative exon. We will annotate the exon skipping event with domain, sites and topology information. The first step is to obtain a maser object containing SRSF6 splicing information, and then map transcripts to splice events.
...
```{r, warning = FALSE, message = FALSE}
# Map transcripts to splice events
srsf6_mapped <- mapTranscriptsToEvents(srsf6_events, ens_gtf)

```

If transcript mapping worked correctly, Ensembl and Uniprot identifiers will be added to splice events. Possible `NA` values indicates non-protein coding transcripts. In this case, the splicing involves two Ensembl transcripts coding for the Q13247 isoform of SRSF6. 
...
Now we are ready to call `mapProteinFeaturesToEvents()` for annotation. Feature annotation can be interactively displayed in a web browser using `display()` or retrieved as a `data.frame` using `annot()`.  
...
RIPK2 has an exon skipping event in the hypoxia dataset. Following the example above, we map transcripts to splice events and annotate protein features overlapping the splice event. We find out that the alternative exon overlaps the kinase domain of the protein, thus possibly changing the configuration of this domain during hypoxia. The ATP and proton acceptor binding sites are overlapping exons flanking the alternative exon.  
...
plotUniprotKBFeatures(ripk2_annot, type = "SE", event_id = 14319, 
                      ...
                      zoom = F, show_transcripts = TRUE)

```

## BMPR2 example

BMPR2 is plasma membrane protein that also undergoes splicing in the hypoxia dataset. After transcript mapping and annotation, we find ou that alternative exon is located in the cytoplasmic portion of the protein, and next to kinase domain. Setting `zoom = TRUE` might be useful for a detailed analysis.

```{r, warning = FALSE, message = FALSE}

bmpr2_events <- geneEvents(hypoxia_filt, "BMPR2")
bmpr2_mapped <- mapTranscriptsToEvents(bmpr2_events, ens_gtf)
bmrp2_annot <- mapProteinFeaturesToEvents(bmpr2_mapped, 
                           tracks = c("Domain_and_Sites",
                                      "Topology"), 
                           by = "category")
...
```{r, warning = FALSE, message = FALSE}
plotUniprotKBFeatures(bmpr2_mapped, type = "SE", event_id = 25501, 
                      features = c("domain", "topo-dom", "mod-res"), 
                      gtf = ens_gtf, 
                      zoom = F,
